...
后台任意一处 **立即采集** 按钮（概览页 / 采集审核池）点下去之后，这一轮会把配图一起跑完：

1. **先抓源入库**：RSS 抓取 → 去重 → 评分 → 自动审核；
2. **再给本轮新条目下载封面**：
   - 第一轮走「原文直链 + 原文页配图」，几秒一条，图与原文天然一致；
   - 第一轮没配上的，再按标题走图库检索（十几秒一张），每轮只给一半预算，避免整轮被拖住；
   - 封面文件名 `i-<待审id>-cover.jpg`，写在待审条目的 `cover` 字段上，
     审核通过发布时**直接沿用这张图**，不会重复下载、也不会重复占图；
3. **跑完才返回**，按钮显示「下载配图 3/12…」，期间两个采集按钮都锁住；
   再点一次会收到 `409`「上一轮采集还没跑完」，不会有两轮采集抢同一批图。

采完没配到图的条目在列表里显示「待配图」占位，可以单独点 **补图 / 换图**
（`POST /api/v1/admin/inbox/:id/image`，走完整路径，慢一点但尽量配上）。
每轮配图的**张数**与**时间上限**在「自动采集 · 审核 · 发布规则」里可调
（每轮采集下载配图张数、每轮配图时间上限），进度可查 `GET /api/v1/admin/pipeline/status`。

这些图同样落在 `public/img/news/`，因此每天 22:30 的同步任务会**连数据带图片一起推到 GitHub**。
