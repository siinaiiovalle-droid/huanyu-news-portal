...
1. **商城辅助入口仍是空壳 `onTap/onPressed: () {}`**：商详底部「客服」「店铺」「分享」、订单页搜索/客服、订单 chip 回调、钱包九宫格（卡包/账单/花呗/理财/公益）、消息页搜索与「+」、我的页扫码与设置、视频页若干按钮。建议接 `widgets/common.dart` 的通用空态或跳对应占位页，而不是静默无响应。
